...
BridgeCrux is the crux operating layer for this agentic application. For any work involving task-signal routing, canonical crux content, handler bindings, deterministic validation or processes, operations, persistence, memory, reports, model or channel adapters, conformance, or end-to-end agent behavior, use the installed \`$use-bridgecrux-primitives\` skill. In the BridgeCrux repository, the skill sources live under \`skills/\`; consumer installations copy their verified bundles into the configured skills root.
